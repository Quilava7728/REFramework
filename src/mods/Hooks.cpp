...
// A  = SetEvent(+0x3a80)
// Build #31E:
// Independent timestamp samples from both object-loop hooks.
thread_local uint64_t g_mhs3_object_loop_begin_clock_us = 0;
thread_local uint64_t g_mhs3_object_loop_end_clock_us = 0;
...
std::optional<std::string> Hooks::hook_mhs3_waitrendering_callsites() {
    ...
    // BUILD31E:
    // Both known-safe hook locations independently sample the clock.
    constexpr uintptr_t producer_object_loop_begin_clock_rva = 0x03fee8a;
    constexpr uintptr_t producer_object_loop_end_clock_rva   = 0x03fef06;
    ...
    if (!m_mhs3_producer_object_loop_begin_clock_hook) {
        return "Failed to install MHS3 Build #31E begin-clock probe";
    }

    m_mhs3_producer_object_loop_end_clock_hook =
        safetyhook::create_mid(
            (void*)(base + producer_object_loop_end_clock_rva),
            &Hooks::mhs3_producer_object_loop_end_clock
        );

    if (!m_mhs3_producer_object_loop_end_clock_hook) {
        return "Failed to install MHS3 Build #31E end-clock probe";
    }

    spdlog::info(
        "[MHS3 BUILD31E] both clock probes installed: begin=0x{:x} end=0x{:x}",
        base + producer_object_loop_begin_clock_rva,
        base + producer_object_loop_end_clock_rva
    );
...
void Hooks::mhs3_producer_object_loop_end_clock(
    safetyhook::Context& context
) {
    (void)context;

    g_mhs3_object_loop_end_clock_us =
        mhs3_steady_now_us();
}
