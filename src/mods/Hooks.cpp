...
// A  = SetEvent(+0x3a80)
// Build #31G4:
// Minimal paired timing plus marker increment only.
...
std::optional<std::string> Hooks::hook_mhs3_waitrendering_callsites() {
    ...
    constexpr uintptr_t producer_object_loop_begin_marker_increment_rva = 0x03fee8a;
    constexpr uintptr_t producer_object_loop_end_marker_increment_rva   = 0x03fef06;

    m_mhs3_producer_object_loop_begin_marker_increment_hook =
        safetyhook::create_mid(
            (void*)(base + producer_object_loop_begin_marker_increment_rva),
            &Hooks::mhs3_producer_object_loop_begin_marker_increment
        );

    if (!m_mhs3_producer_object_loop_begin_marker_increment_hook) {
        return "Failed to install MHS3 Build #31G4 begin-marker-increment probe";
    }

    m_mhs3_producer_object_loop_end_marker_increment_hook =
        safetyhook::create_mid(
            (void*)(base + producer_object_loop_end_marker_increment_rva),
            &Hooks::mhs3_producer_object_loop_end_marker_increment
        );

    if (!m_mhs3_producer_object_loop_end_marker_increment_hook) {
        return "Failed to install MHS3 Build #31G4 end-marker-increment probe";
    }

    spdlog::info(
        "[MHS3 BUILD31G4] object-loop marker-increment probes installed: "
        "begin=0x{:x} end=0x{:x}",
        base + producer_object_loop_begin_marker_increment_rva,
        base + producer_object_loop_end_marker_increment_rva
    );
...
void Hooks::mhs3_producer_object_loop_begin_marker_increment(
...
void Hooks::mhs3_producer_object_loop_end_marker_increment(
    safetyhook::Context& context
) {
    (void)context;
