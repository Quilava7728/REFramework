...
// A  = SetEvent(+0x3a80)
// Build #31F:
// Minimal paired object-loop timing state.
thread_local uint64_t g_mhs3_object_loop_begin_us = 0;
thread_local uint64_t g_mhs3_object_loop_elapsed_us = 0;
...
std::optional<std::string> Hooks::hook_mhs3_waitrendering_callsites() {
    ...
    // BUILD31F:
    // Minimal paired timing across the two known-safe hook sites.
    constexpr uintptr_t producer_object_loop_begin_elapsed_rva = 0x03fee8a;
    constexpr uintptr_t producer_object_loop_end_elapsed_rva   = 0x03fef06;

    m_mhs3_producer_object_loop_begin_elapsed_hook =
        safetyhook::create_mid(
            (void*)(base + producer_object_loop_begin_elapsed_rva),
            &Hooks::mhs3_producer_object_loop_begin_elapsed
        );

    if (!m_mhs3_producer_object_loop_begin_elapsed_hook) {
        return "Failed to install MHS3 Build #31F begin-elapsed probe";
    }

    m_mhs3_producer_object_loop_end_elapsed_hook =
        safetyhook::create_mid(
            (void*)(base + producer_object_loop_end_elapsed_rva),
            &Hooks::mhs3_producer_object_loop_end_elapsed
        );

    if (!m_mhs3_producer_object_loop_end_elapsed_hook) {
        return "Failed to install MHS3 Build #31F end-elapsed probe";
    }

    spdlog::info(
        "[MHS3 BUILD31F] paired elapsed probes installed: begin=0x{:x} end=0x{:x}",
        base + producer_object_loop_begin_elapsed_rva,
        base + producer_object_loop_end_elapsed_rva
    );
...
void Hooks::mhs3_producer_object_loop_begin_elapsed(
...
void Hooks::mhs3_producer_object_loop_end_elapsed(
    safetyhook::Context& context
) {
    (void)context;
