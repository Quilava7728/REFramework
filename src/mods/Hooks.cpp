...
// A  = SetEvent(+0x3a80)
// Build #31G2:
// Minimal paired timing plus elapsed total only.
...
std::optional<std::string> Hooks::hook_mhs3_waitrendering_callsites() {
    ...
    // BUILD31G2:
    // Proven elapsed timing plus total accumulation only.
    constexpr uintptr_t producer_object_loop_begin_total_rva = 0x03fee8a;
    constexpr uintptr_t producer_object_loop_end_total_rva   = 0x03fef06;

    m_mhs3_producer_object_loop_begin_total_hook =
        safetyhook::create_mid(
            (void*)(base + producer_object_loop_begin_total_rva),
            &Hooks::mhs3_producer_object_loop_begin_total
        );

    if (!m_mhs3_producer_object_loop_begin_total_hook) {
        return "Failed to install MHS3 Build #31G2 begin-total probe";
    }

    m_mhs3_producer_object_loop_end_total_hook =
        safetyhook::create_mid(
            (void*)(base + producer_object_loop_end_total_rva),
            &Hooks::mhs3_producer_object_loop_end_total
        );

    if (!m_mhs3_producer_object_loop_end_total_hook) {
        return "Failed to install MHS3 Build #31G2 end-total probe";
    }

    spdlog::info(
        "[MHS3 BUILD31G2] object-loop total-only probes installed: "
        "begin=0x{:x} end=0x{:x}",
        base + producer_object_loop_begin_total_rva,
        base + producer_object_loop_end_total_rva
    );
...
void Hooks::mhs3_producer_object_loop_begin_total(
...
void Hooks::mhs3_producer_object_loop_end_total(
    safetyhook::Context& context
) {
    (void)context;
