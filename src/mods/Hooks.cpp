...
// A  = SetEvent(+0x3a80)
// Build #31G1:
// Minimal paired timing plus completed-loop count only.
...
std::optional<std::string> Hooks::hook_mhs3_waitrendering_callsites() {
    ...
    // BUILD31G1:
    // Proven elapsed timing plus count only.
    constexpr uintptr_t producer_object_loop_begin_count_rva = 0x03fee8a;
    constexpr uintptr_t producer_object_loop_end_count_rva   = 0x03fef06;

    m_mhs3_producer_object_loop_begin_count_hook =
        safetyhook::create_mid(
            (void*)(base + producer_object_loop_begin_count_rva),
            &Hooks::mhs3_producer_object_loop_begin_count
        );

    if (!m_mhs3_producer_object_loop_begin_count_hook) {
        return "Failed to install MHS3 Build #31G1 begin-count probe";
    }

    m_mhs3_producer_object_loop_end_count_hook =
        safetyhook::create_mid(
            (void*)(base + producer_object_loop_end_count_rva),
            &Hooks::mhs3_producer_object_loop_end_count
        );

    if (!m_mhs3_producer_object_loop_end_count_hook) {
        return "Failed to install MHS3 Build #31G1 end-count probe";
    }

    spdlog::info(
        "[MHS3 BUILD31G1] object-loop count-only probes installed: "
        "begin=0x{:x} end=0x{:x}",
        base + producer_object_loop_begin_count_rva,
        base + producer_object_loop_end_count_rva
    );
...
void Hooks::mhs3_producer_object_loop_begin_count(
...
void Hooks::mhs3_producer_object_loop_end_count(
    safetyhook::Context& context
) {
    (void)context;
